...
from miscellaneous import throwError, throwWarning, charToString, readArrayEntries, writeArrayEntries, sourceDirectoryPath, includePaths,  libraryPaths, checkArraySizes
import analysis
...
def readMMFHeader(file,VERBOSE=True):
    ...
    if VERBOSE: print "Reading the header of the MMF file '%s' ... " % tempName,
    f = open(tempName, 'rb')
    header.fromfile( f )
    f.close()
...
def readMMFData(file,HEADER=True,VERBOSE=True):
    ...
    for i in range(header.noMMFFiles):
        ...
        tempName = file
        if header.noMMFFiles!=1: tempName = MMMFMultipleFiles( file, i )
...
def writeMMFData(file,header,data,VERBOSE=True,ERROR_CHECK=True):
    ...
    if VERBOSE: print "Writing the MMF data to the file '%s' ... " % file,
...
def getHeaderType(file):
    ...
    if buffer1[0]==256 or buffer1[0]==8 or swappedBuffer==256 or swappedBuffer==8:
        headerType = 'Gadget'
        return headerType
    ...
    if buffer1!=1024: return headerType # returns 'Unknown'
    header = readMMFHeader(file,VERBOSE=False)
    if header.FILE_ID in FILE_ID.keys():
        return FILE_ID[ header.FILE_ID ]       # some files don't have a valid value for this argument, so proceed further
    elif header.filter//10==header.feature:    # header.filter//10==header.feature only for MMF files
        headerType = 'MMF'
    else:
        headerType = 'Density'
    return headerType
...
def NEXUSCombineEnvironments(nodeCleanData,filaCleanData,wallCleanData,VERBOSE=True):
    ...
    if VERBOSE: print "Combining the node, filament and wall environments into a single data structure ..."
...
def NEXUSEnvironmentProperties(allCleanData,propertiesData,env,masking=(True,True,True),VERBOSE=True):
    ...
    if env not in envList:
        throwError( "Invalid value for the 'env' argument in the '%s' function. Allowed values for this argument are: %s" % (funcName,str(envList)) )
    elif env in ['fila','wall']:
        env = [3,2][int(env=='wall')]
    envName = ['filament','wall'][int(env==2)]
    if propertiesData.ndim<=1:
        throwError( "The argument 'propertiesData. in the '%s' function needs to be at least a 2D numpy array. The last dimension gives the number of properties for each grid cell." )
    
    # get the mask and check compatibility with the input data
    if VERBOSE: print "Getting the %s properties on a grid cell data format ..." % envName
    ...
    if masking[1] and env==2:
        ...
        if maskText=='': maskText = 'filament'
        else: maskText = '%s + filament' % maskText
    noElements, noDatas = np.sum(select), propertiesData.size/propertiesData.shape[-1]
    if noElements!=noDatas:
        throwError( "The number of valid environment grid cells and the size of the properties data does not match. There are %i valid environment grid cells while there are %i properties data. The requested environment is '%s' using %s masks." % (noElements,noDatas,envName,maskText) )
