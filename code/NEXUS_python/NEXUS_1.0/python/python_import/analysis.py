...
dtype2ctype = {np.dtype(np.float64): 'double', np.dtype(np.float32): 'float', np.dtype(np.int32): 'int', np.dtype(np.uint64): 'size_t', np.dtype(np.int16): 'short'}


def MinMax(data):
    """ Computes the min and maximum of a data set. It return two tuples: (minValue,maxValue) and (minIndex,maxIndex). """
    if data.size <=0: throwError( "The numpy array supplied as argument to the 'MinMax' function must have at least one elemnt." )
...
def binBoundaries(dataRange,no_bins,bin_type='linear'):
    ...
    if bin_type not in ['linear','logarithm']: throwError( "Unknown bin type in function 'binBoundaries'. You inserted 'bin_type'='%s', but the function recognizes only the following values '%s'." % (bin_type,str(['linear','logarithm'])) )
    output = None
    if bin_type is 'linear':
        output = np.linspace( dataRange[0], dataRange[1], num=no_bins+1, endpoint=True )
    if bin_type is 'logarithm':
        output = np.logspace( np.log10(dataRange[0]), np.log10(dataRange[1]), num=no_bins+1, endpoint=True )
    return output
...
def binValues(dataRange,no_bins,bin_type='linear'):
    ...
    if bin_type not in ['linear','logarithm']: throwError( "Unknown bin type in function 'binValues'. You inserted 'bin_type'='%s', but the function recognizes only the following values '%s'." % (bin_type,str(['linear','logarithm'])) )
    binBoundary = binBoundaries( dataRange, no_bins, bin_type=bin_type )
    output = np.zeros( no_bins, binBoundary.dtype )
    if bin_type is 'linear':
        output[:] = (binBoundary[0:no_bins] + binBoundary[1:no_bins+1]) / 2.
    if bin_type is 'logarithm':
        output[:] = np.sqrt(binBoundary[0:no_bins] * binBoundary[1:no_bins+1])
    return output


def Histogram(data,no_bins=100,bins=None,Range=(None,None),bin_type="linear",weights=None):
    ...
    if bins is None:
        if no_bins <= 0:
            throwError( "The 'no_bins' parameter of function 'Histogram' must be at least 1." )
        if bin_type not in ['linear','logarithm']:
            throwError( "Unrecognized histogram '%s' bin type in function 'Histogram'. The parameter 'bin_type' can be 'linear' or 'logarithm' to have linearly/logarithmically spaced bins." % bin_type )
        ...
        if MinRange is None or MaxRange is None:
            ...
            if MinRange is None: MinRange = _min*.99
            if MaxRange is None: MaxRange = _max*1.01
        ...
        finalShape = data.shape
        if data.size!=0: data.shape = -1
        if weights is not None and weights.size is not data.size:
            throwError( "The 'data' and 'weights' input arrays of the 'Histogram' function must have the same size. 'data.size'=%s and 'weights.size'=%s." % (str(data.size),str(weights.size)) )
        if bin_type is 'linear':
            inline( HistogramCode_linearBin, ['data','MinRange','MaxRange', 'no_bins','bins','histData','weights'], type_converters=converters.blitz, headers=['<cmath>'] )
        if bin_type is 'logarithm':
            if MinRange*MaxRange <= 0.: throwError( "Cannot take logarithmic bin steps when the product of the minimum and maximum bin limits is 0 or negative. 'MinRange'=%s and 'MaxRange'=%s." % (str(MinRange),str(MaxRange)) )
            inline( HistogramCode_logarithmicBin, ['data','MinRange','MaxRange', 'no_bins','bins','histData','weights'], type_converters=converters.blitz, headers=['<cmath>'] )
        if data.size!=0: data.shape = finalShape
...
def CumulativeSum(data,order="ascending",out_type=None):
    ...
    if order not in ["ascending", "descending"]: throwError( "The cumulative ordering '%s' is not recognized in function 'CumulativeSum'. The function recognizes only the values 'ascending'/'descending' to compute the ascending/descending cumulative sum." % str(order) )
...
def PDF(data,pdfRange=(None,None),bin_type="linear"):
    ...
    if (pdfRange[0] is None) or (pdfRange[1] is None):
        throwError( "You must specify the bin limits for the data in function 'PDF' using the 'pdfRange' argument." )
    ...
    if bin_type is "linear":
        ...
        if pdfRange.size!=data.size+1:
            throwError( "In function 'PDF'. For bin_types='variable' the 'pdfRange' argument should give the bin boundaries used to compute the data. So the 'pdfRange' argument should have length 'data'.size+1. But length('pdfRange')=%i while length('data')=%i." % (pdfRange.size,data.size) )
        binsSpacing[:] = pdfRange[1:] - pdfRange[:-1]
    else:
        throwError( "In function 'PDF'. Unknown value for the 'bin_type' argument. Allowed values are %s." % bin_type_list )
...
def Median(data,N=100,minSize=5):
    ...
    if data.size<minSize:       # the data is not large enough to get an accurate error for it
...
def Percentile(data,percentile=(25.,75.),N=100):
...
def DistributionPercentile(X,Y,percentile=(25.,75.)):
...
def CorrelationMatrix(data,N=100,minColumns=5):
    ...
    if data.shape[0]<minColumns:       # the data is not large enough to get an accurate error for it
        return correlation, correlation
    correlationSet = np.zeros( (N,correlation.shape[0]*correlation.shape[1]), np.float64 )
    indices = np.arange( data.shape[0] )
    for i in range(N):
        select = Bootstrap(indices)
        correlationSet[i,:] = np.corrcoef( data[select,:], rowvar=0 ).flatten()
    correlationError = np.std( correlationSet.reshape(N,-1), axis=0 ).reshape( correlation.shape[0], correlation.shape[1] )
...
def randomSample_direction(noPoints,noBins):
    X = np.random.rand(noPoints,3)*2.-1.
    Y = np.random.rand(noPoints,3)*2.-1.
    temp = np.abs( scalarProduct( X, Y ) )
    counts, discard = np.histogram( temp, bins=noBins, range=(0.,1.) )
    return counts
...
def dataCorrelation_direction(data,noBins,noRepeats):
    ...
    hist, discard = np.histogram( data, bins=noBins, range=(0.,1.) )
...
def LeastSquare_linearFit(X,Y,constant=True):
...
def LeastSquare_nonlinearFit_singleParameter(X,Y,func,guess,relMaxError=1.e-3,noMaxSteps=100):
    ...
    while iteration<noMaxSteps and relError>relMaxError:
        ...
        min = error.min()
        if min==error[0]: param = pTry[0]
        elif min==error[1]: param, step = pTry[1], step/4
        elif min==error[2]: step = step/4
        elif min==error[3]: param, step = pTry[3], step/4
        elif min==error[4]: param = pTry[4]
        iteration += 1
        relError = step/param
    
    success = True
    if iteration==noMaxSteps: success = False
    errorSquare = ( (Y - func(X,param))**2 ).sum()
    return param, errorSquare, success


def LeastSquare_nonlinearFit(X,Y,func,deriv,guess,relMaxError=1.e-3,noMaxSteps=100):
    ...
    if len(guess)!=len(deriv):
        throwError( "In function '%s'. Inconsistent number of initial guess for the parameters and partial derivatives of the fit function. There are %i initial guess parameters and %i partial function derivatives." % (funcName,len(guess),len(deriv)) )
    iteration, relError = 0, 10.*relMaxError
...
def PointDistribution(data,noBins=(10,10),xRange=None,yRange=None,bin_type='linear'):
    ...
    if bin_type not in ['linear','logarithm']: throwError( "Unknown bin type in function 'PointDistribution'. You inserted 'bin_type'='%s', but the function recognizes only the following values '%s'." % (bin_type,str(['linear','logarithm'])) )
    if data.ndim!=2 or data.shape[1]!=2: throwError( "The input data in function 'PointDistribution' must be a 2D numpy array with 2 columns. The input data is a %i-D numpy array with %i columns." % (data.ndim,data.shape) )
    if xRange is None:
    ...
    if bin_type is 'linear':
        inline( PointDistributionCode_linearBin, ['data','Range','noBins', 'bins'], type_converters=converters.blitz, headers=['<cmath>'] )
    elif bin_type is 'logarithm':
        if MinRange*MaxRange <= 0.: throwError( "Cannot take logarithmic bin steps when the product of the minimum and maximum bin limits is 0 or negative. 'MinRange'=%s and 'MaxRange'=%s." % (str(MinRange),str(MaxRange)) )
        inline( PointDistributionCode_logarithmicBin, ['data','Range','noBins', 'bins'], type_converters=converters.blitz, headers=['<cmath>'] )
...
def FindContourValue(data,enclosed_fraction,bin_type='linear'):
    ...
    if bin_type not in ['linear','logarithm']: throwError( "Unknown bin type in function 'FindContourValue'. You inserted 'bin_type'='%s', but the function recognizes only the following values '%s'." % (bin_type,str(['linear','logarithm'])) )
    
    # Compute the histogram of the data
    temp = data.reshape(-1)
    noBins = 1000
    histogram, histogramBins = Histogram(temp,no_bins=noBins,bin_type=bin_type,weights=None)
