...
NO_SCALAR_COMPONENTS = 1


densityMethod = {1:'DTFE', 2:'TSC', 3:'SPH', np.uint64(-1):'Unknown', -1:'Unknown' }
...
def readDensityHeader(file,VERBOSE=True):
    ...
    if VERBOSE: print "Reading the header of the density file '%s' ... " % tempName,
    f = open(tempName, 'rb')
    header.fromfile( f )
...
def readDensityData(file,HEADER=True,VERBOSE=True):
    ...
    for i in range(header.noDensityFiles):
        if VERBOSE: print "Reading the data in the density file '%s' which is file %i of %i files ... " % (tempName,i+1,header.noDensityFiles)
        tempName = file
...
def writeDensityData(file,header,data,VERBOSE=True):
    ...
    if VERBOSE: print "Writing the density data to the file '%s' ... " % file,
